...
INCLUDEDEATHRD = ADDON.getSetting('includeeathrd')
INCLUDEPLACE  = ADDON.getSetting('includeplace')
INCLUDENEPTUNE = ADDON.getSetting('includeneptune')
INCLUDEKNOB  = ADDON.getSetting('includeknob')
INCLUDEONECHAN = ADDON.getSetting('includeonechan')
INCLUDEDEATH   = ADDON.getSetting('includedeath')
INCLUDEDEATHRD = ADDON.getSetting('includedeathrd')
...
def getCacheSize():
	...
	dbfiles   = [
		...
		(os.path.join(DATABASE,  'DSRDcache.db'))]
	cachelist = [
		(PROFILEADDONDATA),
		(ADDONDATA),
