...
def cleanupBackup():
	...
	list = []; filelist = []
	if len(folder) == 0:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Backup Pfad: Empty[/COLOR]" % (COLOR2))
		return
	for item in sorted(folder, key=os.path.getmtime):
		...
		if os.path.isdir(item): 
			...
			list.append(base)
	list = ['--- Loesche alle Eintraege ---'] + list
	...
	if selected == -1:
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to clean up all items in your 'My_Builds' folder?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR green]Clean Up[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
			...
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]geloeschte Dateien: [COLOR %s]%s[/COLOR] / Verzeichnisse:[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, clearedfiles, COLOR1, clearedfolders))
		else:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Clean Up Cancelled![/COLOR]" % COLOR2)
	else:
			...
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if yes:
						...
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Bereinige Packages: Erfolg![/COLOR]' % COLOR2)
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Bereinige Packages: Erfolg: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			#else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Clear Packages: None Found![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Clear Packages: Error![/COLOR]' % COLOR2)
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
...
def clearCache(over=None):
	...
	LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Bereinigung Cache: %s Dateien entfernt[/COLOR]' % (COLOR2, delfiles))
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Loesche Thumbs: %s Dateien / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	#else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Loesche Thumbs: None Found![/COLOR]' % COLOR2)
