...
def index():
	...
	if len(BUILDNAME) > 0:
		...
		if not themefile == False:
			addFile('Keine' if BUILDTHEME == "" else BUILDTHEME, 'theme', BUILDNAME, themeit=THEME5)
	else: addDir('Keine', 'builds', themeit=THEME4)
	...
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Einstellungen'      ,'settings', icon=ICONSETTINGS, themeit=THEME2)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		addFile('Installierte %s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
		addFile('Version auswaehlen', '', icon=ICONBUILDS, themeit=THEME2)
		...
		addFile('Installierte %s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
		addFile('Version auswaehlen', '', icon=ICONBUILDS, themeit=THEME2)
		#addDir ('Daten sichern'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		if third == True:
			for item in addin:
		...
		if len(match) >= 1:
			...
			if adultcount > 0:
				addFile('Nur Adult builds verfuegbar', '', icon=ICONBUILDS, themeit=THEME3)
				addFile('Enable Show Adults in Addon Settings > Misc', '', icon=ICONBUILDS, themeit=THEME3)
			else:
				addFile('Keine Builds von %s verfuegbar' % ADDONTITLE, '', icon=ICONBUILDS, themeit=THEME3)
		else: addFile('Builds Datei nicht korrekt formatiert.', '', icon=ICONBUILDS, themeit=THEME3)
	setView('files', 'viewType')

def viewBuild(name):
	WORKINGURL = wiz.workingURL(BUILDFILE)
	if not WORKINGURL == True:
		addFile('Url der Textdatei nicht gueltig', '', themeit=THEME3)
		addFile('%s' % WORKINGURL, '', themeit=THEME3)
		return
	if wiz.checkBuild(name, 'version') == False: 
		addFile('Fehler beim Lessen der Textdatei.', '', themeit=THEME3)
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		if BUILDNAME == name and version > BUILDVERSION:
			build = '%s [COLOR red][Aktuell v%s][/COLOR]' % (build, BUILDVERSION)
		addFile(build, '', description=description, fanart=fanart, icon=icon, themeit=THEME4)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addDir ('Daten sichern',       'savedata', icon=ICONSAVE,     themeit=THEME2)
		addFile('Build Informationen',    'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME2)
		if not preview == "http://": addFile('Video Vorschau ansehen', 'buildpreview', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
		...
		if warning == True:
			addFile('[I]Build entworfen fuer Kodi Version %s(installiert: %s)[/I]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile(wiz.sep('INSTALLIEREN'), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('Jetzt installieren'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME2)
		#addFile('Standard Installation', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
		if not gui == 'http://': addFile('guiFix anwenden'    , 'install', name, 'gui'     , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def maintMenu(view=None):
	on = '[B][COLOR green]AN[/COLOR][/B]'; off = '[B][COLOR red]AUS[/COLOR][/B]'
	...
	errorsfound = str(errorsinlog) + ' Fehler gefunden' if errorsinlog > 0 else 'None Found'
	wizlogsize = ': [COLOR red]nicht gefunden[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR green]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
	if includeall == 'true':
		includebob = 'true'
		includepho = 'true'
		includespe = 'true'
	...
	feq        = ['Immer', 'Taeglich', 'Alle 3 Tage', 'Woechentlich']
...
def saveMenu():
	...
	addDir ('Einstellungen Trakt Daten',               'trakt',                icon=ICONTRAKT, themeit=THEME1)
	addDir ('Einstellungen Real Debrid',              'realdebrid',           icon=ICONREAL,  themeit=THEME1)
	#addDir ('Behalte Login Info',               'login',                icon=ICONLOGIN, themeit=THEME1)
	...
	addFile('Behalte Trakt: %s' % trakt.replace('true',on).replace('false',off)                       ,'togglesetting', 'keeptrakt',      icon=ICONTRAKT, themeit=THEME1)
	addFile('Behalte Real Debrid: %s' % real.replace('true',on).replace('false',off)                  ,'togglesetting', 'keepdebrid',     icon=ICONREAL,  themeit=THEME1)
	...
	addFile('Behalte installierte Repos: %s' % repos.replace('true',on).replace('false',off)           ,'togglesetting', 'keeprepos',      icon=ICONSAVE,  themeit=THEME1)
	addFile('Behalte My \'WhiteList\': %s' % whitelist.replace('true',on).replace('false',off)        ,'togglesetting', 'keepwhitelist',  icon=ICONSAVE,  themeit=THEME1)
	if whitelist == 'true':
...
def changeFeq():
	feq        = ['Bei jedem Start', 'Taeglich', 'Alle drei Tage', 'Woechentlich']
	change     = DIALOG.select("[COLOR %s]Wie of soll Auto Cleanup beim Start aufraeumen?[/COLOR]" % COLOR2, feq)
	if not change == -1: 
		wiz.setS('autocleanfeq', str(change))
		wiz.LogNotify('[COLOR %s]Auto Clean Up[/COLOR]' % COLOR1, '[COLOR %s]Intervall jetzt %s[/COLOR]' % (COLOR2, feq[change]))
...
def buildWizard(name, type, theme=None, over=False):
	if over == False:
		...
		if testbuild == False:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Kann Build nicht finden[/COLOR]" % COLOR2)
			return
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Soll GuiFix angewendet werden fuer:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR green]Fix anwenden[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]WARNUNG!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build ist aktuell nicht installiert." % (COLOR2, COLOR1, name), "guiFix trotzdem anwenden?.[/COLOR]", nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR green]Fix anwenden[/COLOR][/B]')
		if yes:
			buildzip = wiz.checkBuild(name,'gui')
			zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
			if not wiz.workingURL(buildzip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]GuiFix: Ungueltige Zip Url![/COLOR]' % COLOR2); return
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Herunterladen von GuiFix:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Bitte warten')
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Gui fix wurde installiert.  Profil neuladen oder Kodi neustarten?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Profil neuladen[/COLOR][/B]", nolabel="[B][COLOR green]Kodi neustarten[/COLOR][/B]")
			if todo == 1: wiz.reloadFix()
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]OK klicken um Kodi neuzustarten[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if warning == True:
			yes_pressed = DIALOG.yesno("%s - [COLOR red]WARNUNG!![/COLOR]" % ADDONTITLE, '[COLOR %s]Moeglicherweise erscheint dsa Skin nicht korrekt' % COLOR2, 'When installing a %s build on a Kodi %s install' % (wiz.checkBuild(name, 'kodi'), KODIV), 'Would you still like to install: [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Yes, Install[/COLOR][/B]')
		else:
			if not over == False: yes_pressed = 1
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Dieses installieren:' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Nein[/COLOR][/B]',yeslabel='[B][COLOR green]Installieren[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			buildzip = wiz.checkBuild(name, 'url')
			zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
			if not wiz.workingURL(buildzip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Build Installation: Ungueltige Zip Url![/COLOR]' % COLOR2); return
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Herunterladen:[/B][/COLOR] [COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')),'', 'Bitte warten')
			...
			if int(float(percent)) > 0:
				...
				if int(float(errors)) > 0:
					yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild( name,'version')), 'Erledigt: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Fehler anzeigen?[/COLOR]', nolabel='[B][COLOR red]Nein[/COLOR][/B]', yeslabel='[B][COLOR green]Fehler anzeigen[/COLOR][/B]')
					if yes:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Wollen Sie [COLOR %s]Kodi neustarten[/COLOR] oder [COLOR %s]Profil neuladen[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neuladen[/COLOR][/B]", nolabel="[B][COLOR green]Kodi neustarten[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
				else: wiz.killxbmc(True)
			else:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox("%s: Fehler bei Build Installation" % ADDONTITLE, error)
		else:
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Das Build [COLOR %s]%s[/COLOR] kommt mit [COLOR %s]%s[/COLOR] unterschiedlichen Themes" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Ein Theme davom installieren?[/COLOR]", yeslabel="[B][COLOR green]Installiere Themes[/COLOR][/B]", nolabel="[B][COLOR red]Themes abbrechen[/COLOR][/B]"):
						wiz.log("Theme Liste: %s " % str(themelist))
						ret = DIALOG.select(ADDONTITLE, themelist)
						wiz.log("Zu installierendes Theme: %s" % ret)
			...
			else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Installation: Nicht gefunden![/COLOR]' % COLOR2)
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Folgendes Theme installieren:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'fuer [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR green]Installiere Theme[/COLOR][/B]", nolabel="[B][COLOR red]Themes abbrechen[/COLOR][/B]")
		if installtheme:
			themezip = wiz.checkTheme(name, theme, 'url')
			zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
			if not wiz.workingURL(themezip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Installation: Ungueltige Zip Url![/COLOR]' % COLOR2); return False
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Herunterladen:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, theme),'', 'Bitte warten')
			lib=os.path.join(PACKAGES, '%s.zip' % zipname)
			try: os.remove(lib)
			except: pass
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s] " % COLOR2, "Alles zuruecksetzen", "bevor das Backup eingespielt wird?[/COLOR]", nolabel='[B][COLOR red]Nein[/COLOR][/B]', yeslabel='[B][COLOR green]Zuruecksetzen[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s] " % COLOR2, "Alles zuruecksetzen", "bevor [COLOR %s]%s[/COLOR] installiert wird?" % (COLOR1, install), nolabel='[B][COLOR red]Nein[/COLOR][/B]', yeslabel='[B][COLOR green]Zuruecksetzen[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s] " % COLOR2, "Alles zuruecksetzen?[/COLOR]", nolabel='[B][COLOR red]Nein[/COLOR][/B]', yeslabel='[B][COLOR green]Zuruecksetzen[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
