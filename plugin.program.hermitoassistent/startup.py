	...
	if len(match) > 0:
		...
		if version > BUILDVERSION:
			if DISABLEUPDATE == 'false':
				wiz.log("[Pruefe Updates] [Installierte Version: %s] [Aktuelle Version: %s] oeffne Update Fenster" % (BUILDVERSION, version), xbmc.LOGNOTICE)
				notify.updateWindow(BUILDNAME, BUILDVERSION, version, icon, fanart)
			else: wiz.log("[Pruefe Updates] [Installierte Version: %s] [Aktuelle Version: %s] Update Fenster deaktiviert" % (BUILDVERSION, version), xbmc.LOGNOTICE)
		else: wiz.log("[Pruefe Updates] [Installierte Version: %s] [Aktuelle Version: %s]" % (BUILDVERSION, version), xbmc.LOGNOTICE)
	else: wiz.log("[Pruefe Updates] FEHLER: Kann Build Version in Build Datei nicht finden", xbmc.LOGERROR)

def checkSkin():
	wiz.log("[Build Pruefung] Ungueltiger Skin Check Start")
	DEFAULTSKIN   = wiz.getS('defaultskin')
	DEFAULTNAME   = wiz.getS('defaultskinname')
	DEFAULTIGNORE = wiz.getS('defaultskinignore')
	gotoskin = False
	if not DEFAULTSKIN == '':
		if os.path.exists(os.path.join(ADDONS, DEFAULTSKIN)):
			if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es sieht aus als ob das Skin zu [COLOR %s]%s[/COLOR] zurueckgesetzt wurde" % (COLOR2, COLOR1, SKIN[5:].title()), "Skin wechseln zu:[/COLOR]", '[COLOR %s]%s[/COLOR]' % (COLOR1, DEFAULTNAME)):
				gotoskin = DEFAULTSKIN
				gotoname = DEFAULTNAME
			else: wiz.log("Skin nicht zurueckgesetzt", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true'); gotoskin = False
	...
	if DEFAULTSKIN == '':
		...
		for folder in glob.glob(os.path.join(ADDONS, 'skin.*/')):
			...
			if os.path.exists(xml):
				...
				else: wiz.log("ID nicht gefunden fuer %s" % folder, xbmc.LOGNOTICE)
			else: wiz.log("ID nicht gefunden fuer %s" % folder, xbmc.LOGNOTICE)
		if len(skinlist) > 0:
			if len(skinlist) > 1:
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es sieht aus als ob das Skin zu [COLOR %s]%s[/COLOR] zurueckgesetzt wurde" % (COLOR2, COLOR1, SKIN[5:].title()), "Verfuegbare Skins anzeigen?[/COLOR]"):
					choice = DIALOG.select("Skin auswaehlen!", skinname)
					if choice == -1: wiz.log("Skin nicht zurueckgesetzt", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true')
					else: 
						gotoskin = skinlist[choice]
						gotoname = skinname[choice]
				else: wiz.log("Skin nicht zurueckgesetzt", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true')
			else:
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es sieht aus als ob das Skin zu [COLOR %s]%s[/COLOR] zurueckgesetzt wurde" % (COLOR2, COLOR1, SKIN[5:].title()), "Skin wechseln zu:[/COLOR]", '[COLOR %s]%s[/COLOR]' % (COLOR1, skinname[0])):
					gotoskin = skinlist[0]
					gotoname = skinname[0]
				else: wiz.log("Skin nicht zurueckgesetzt", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true')
		else: wiz.log("Keine Skins im Addon Verzeichnis gefunden.", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true'); gotoskin = False
	if gotoskin:
	...
	wiz.log("[Build Check] Ungueltiger Skin pruefe Ende", xbmc.LOGNOTICE)
...
if KODIV >= 17:
	...
	if not wiz.getS('kodi17iscrap') == str(NOW):
		...
		wiz.log("Fortsetzenn von Start Up Script")

wiz.log("[Pfad Check] Gestartet", xbmc.LOGNOTICE)
path = os.path.split(ADDONPATH)
if not ADDONID == path[1]: DIALOG.ok(ADDONTITLE, '[COLOR %s]Bitte sicherstellen, dass das plugin Verzeichnis den selben Namen hat wie die ADDON_ID.[/COLOR]' % COLOR2, '[COLOR %s]Plugin ID:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, ADDONID), '[COLOR %s]Plugin Verzeichnis:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, path)); wiz.log("[Pfad Check] ADDON_ID and plugin Verzeichnisname stimmen nicht ueberein. %s / %s " % (ADDONID, path))
else: wiz.log("[Pfad Check] Gut!", xbmc.LOGNOTICE)

if KODIADDONS in ADDONPATH:
	wiz.log("Kopiere Pfad in Addon Verzeichnis", xbmc.LOGNOTICE)
	if not os.path.exists(ADDONS): os.makedirs(ADDONS)
	newpath = xbmc.translatePath(os.path.join('special://home/addons/', ADDONID))
	if os.path.exists(newpath):
		wiz.log("Verzeichnis existiert bereits, raeume auf", xbmc.LOGNOTICE)
...
wiz.log("[Auto Install Repo] Gestartet", xbmc.LOGNOTICE)
if AUTOINSTALL == 'Yes' and not os.path.exists(os.path.join(ADDONS, REPOID)):
	...
	if workingxml == True:
		...
		if len(ver) > 0:
			...
			if workingrepo == True:
				DP.create(ADDONTITLE,'Repo herunterladen...','', 'Bitte warten')
				...
				xbmc.sleep(500)
				wiz.forceUpdate(True)
				wiz.log("[Auto Install Repo] Erfolgreich installiert", xbmc.LOGNOTICE)
			else: 
				wiz.LogNotify("[COLOR %s]Repo Install Error[/COLOR]" % COLOR1, "[COLOR %s]Ungueltige url der zip![/COLOR]" % COLOR2)
				wiz.log("[Auto Install Repo] Konnte keine funktionierende Repo URL erstellen. %s" % workingrepo, xbmc.LOGERROR)
		else:
			wiz.log("Ungueltige URL fuer Repo Zip", xbmc.LOGERROR)
	else: 
		wiz.LogNotify("[COLOR %s]Repo Install Fehler[/COLOR]" % COLOR1, "[COLOR %s]Ungueltige addon.xml Datei![/COLOR]" % COLOR2)
		wiz.log("[Auto Install Repo] Kann addon.xml nicht lesen.", xbmc.LOGERROR)
elif not AUTOINSTALL == 'Yes': wiz.log("[Auto Install Repo] Nicht aktiviert", xbmc.LOGNOTICE)
elif os.path.exists(os.path.join(ADDONS, REPOID)): wiz.log("[Auto Install Repo] Repository schon installiert")

wiz.log("[Auto Update Wizard] Gestartet", xbmc.LOGNOTICE)
if AUTOUPDATE == 'Yes':
...
if INSTALLED == 'true':
	if KODIV >= 17:
		...
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s]%s[/COLOR] [COLOR %s]wurde nicht korrekt installiert!' % (COLOR1, COLOR2, BUILDNAME), 'Installiert: [COLOR %s]%s[/COLOR] / Anzahl Fehler: [COLOR %s]%s[/COLOR]' % (COLOR1, EXTRACT, COLOR1, EXTERROR), 'Erneut versuchen??[/COLOR]', nolabel='[B]Abbrechen[/B]', yeslabel='[B]Erneut versuchen[/B]')
		wiz.clearS('build')
		FAILED = True
		if yes: 
		...
		if not wiz.currSkin() == defaults and not BUILDNAME == "":
			...
			if gui == 'http://':
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Anscheinend wurden die Skin Einstellungen nicht uebernommen fuer das Build" % COLOR2, "Leider wurde kein GUI Fix fuer das Build angewendet", "Build muss neu installiert werden mit anschliessendem Neustart[/COLOR]")
			elif wiz.workingURL(gui):
				yes=DIALOG.yesno(ADDONTITLE, '%s wurde nicht korrekt installiert!' % BUILDNAME, 'Anscheinend wurden die Skin Einstellungen nicht uebernommen fuer das Build.', 'GuiFix anwenden?', nolabel='[B]Abbrechen[/B]', yeslabel='[B]Fix anwenden[/B]')
